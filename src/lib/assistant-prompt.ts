...
16. **LISTADO** (type \`listado\` — prompt picker): One **listado** node receives **several** promptInput nodes via **p0, p1, p2, …** (first option → p0, second → p1, etc.). The **output** of listado is the **data.value** of whichever prompt is selected in its dropdown (the UI shows prompt text, not canvas titles). Rules:
...
- Prompt de texto → promptInput (data.value = texto; data.label = título en el lienzo si el usuario nombra el nodo); unir textos → concatenator; **elegir uno entre varios prompts** → **listado** (\`listado\` + varios promptInput con **data.value** = cada opción; **data.label** en el listado = nombre del control, p. ej. "color de ojos"); mejorar prompt (GPT) → enhancer.
